    ...
    if (trxCode && trxName && createdById) {
        try {
            ...
            if (sameTransactionCode) {
                ...
                let notificationDescription = "Another transaction code is having the same title or code."
                ...
                if (createdTransactionCode) {
                    ...
                    let notificationDescription = "transaction code was created successfully."
                    res.send(createdTransactionCode)
                } else {
                    console.log('transaction code not created')
                    let notificationDescription = "transaction code could not be created."
                    return res.status(406).end(notificationDescription)
                }
            }
            ...
            let notificationDescription = "transaction code could not be created. Please try again after some times."
            return res.status(500).end(notificationDescription)
        }
    } else {
...
transactionCodesRouter.put('/update', async (req, res) => {
    ...
    if (transactionCodeId && newTransactionCode.trxCode && newTransactionCode.trxName) {
        try {
            const conflictingTransactionCode = await prisma.transactionCodes.findFirst({
                where: {
                    OR: [
            ...
            if (conflictingTransactionCode) {
                console.log("same transaction code found with the updated configuration")
                let notificationDescription = "Same transactioncode exists where the name or code is as the updated transaction code"
                ...
                try {
                    ...
                    if (updatedTransactionCode) {
                        ...
                        let notificationDescription = "Please try again after sometimes. transaction code was not updated."
                        return res.status(500).end(notificationDescription)
                    }
                } catch (error) {
                    console.log("transaction code not found")
                    let notificationDescription = "Please check if this transaction code is there or not."
                    return res.status(403).end(notificationDescription)
                }
            }
        } catch (error) {
