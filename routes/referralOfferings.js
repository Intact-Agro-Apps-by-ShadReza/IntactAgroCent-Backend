...
referralOfferingsRouter.post('/create', async (req, res) => {
    const { referralName, rewardType, rewardAmount, rewardDescription, referralFromId } = req.body    

    if (referralName && rewardType && rewardAmount && rewardDescription && referralFromId) {
        try {
            ...
            if (sameReferralOffering) {
                ...
                let notificationDescription = "Another Referral Offering is having the same title and application."
                ...
                if (createdReferralOffering) {
                    ...
                    let notificationDescription = "Referral Offering was created successfully."
                    res.send(createdReferralOffering)
                } else {
                    console.log('Referral Offering not created')
                    let notificationDescription = "Referral Offering could not be created."
                    return res.status(406).end(notificationDescription)
                }
            }
            ...
            let notificationDescription = "Referral Offering could not be created. Please try again after some times."
            return res.status(500).end(notificationDescription)
        }
    } else {
...
referralOfferingsRouter.put('/update', async (req, res) => {
    ...
    if (referralOfferingId && newReferralOffering.referralName && newReferralOffering.rewardType && newReferralOffering.rewardAmount && newReferralOffering.rewardDescription && newReferralOffering.referralFromId) {
        try {
            const conflictingReferralOffering = await prisma.referralOfferings.findFirst({
                where: {
            ...
            if (conflictingReferralOffering) {
                console.log("same Referral Offering found with the updated title and application")
                let notificationDescription = "Same Referral Offering exists where the title and application is as the updated Referral Offering"
                ...
                try {
                    ...
                    if (updatedReferralOffering) {
                        ...
                        let notificationDescription = "Please try again after sometimes. Referral Offering was not updated."
                        return res.status(500).end(notificationDescription)
                    }
                } catch (error) {
                    console.log("Referral Offering not found")
                    let notificationDescription = "Please check if this Referral Offering is there or not."
                    return res.status(403).end(notificationDescription)
                }
            }
        } catch (error) {
