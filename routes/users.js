...
usersRouter.post('/create', async (req, res) => {
    const { username, firstName, lastName, email, phoneNumber, bankAccountNumber, nidCardFrontPicLink, nidCardBackPicLink, address, country, dateOfBirth } = req.body

    if (username && firstName && lastName && email && phoneNumber && bankAccountNumber && nidCardFrontPicLink && nidCardBackPicLink && address && country && dateOfBirth) {
        try {
            ...
            if (sameUser) {
                ...
                let notificationDescription = "Another user is having the same credential configuration."
            ...
            let notificationDescription = "User could not be created. Please try again after some times."
            return res.status(500).end(notificationDescription)
        }
    } else {
...
usersRouter.put('/update', async (req, res) => {
    ...
    if (
        userId && newUser.username && newUser.firstName && newUser.lastName && newUser.email && newUser.phoneNumber && newUser.bankAccountNumber && newUser.nidCardFrontPicLink && newUser.nidCardBackPicLink && newUser.address && newUser.country && newUser.dateOfBirth) {
        try {
            ...
            if (conflictingUser) {
                ...
                let notificationDescription = "Same User exists where the configurations are as the updated User"
                ...
                try {
                    ...
                    if (updatedUser) {
                        ...
                        let notificationDescription = "Please try again after sometimes. User was not updated."
                        return res.status(500).end(notificationDescription)
                    }
                } catch (error) {
                    console.log("User not found")
                    let notificationDescription = "Please check if this User is there or not."
                    return res.status(403).end(notificationDescription)
                }
            }
        } catch (error) {
