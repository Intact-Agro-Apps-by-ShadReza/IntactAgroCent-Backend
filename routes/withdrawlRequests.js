    ...
    if (initiatedById && withdrawlAmount && status && processedById) {
        try {
            ...
            if (createdWithdrawlRequest) {
                ...
                let notificationDescription = "Withdrawl Request was created successfully."
            ...
            let notificationDescription = "Withdrawl Request could not be created. Please try again after some times."
            return res.status(500).end(notificationDescription)
        }
    } else {
...
withdrawlRequestsRouter.put('/update', async (req, res) => {
    ...
    if (withdrawlRequestId && newWithdrawlRequest.initiatedById && newWithdrawlRequest.withdrawlAmount && newWithdrawlRequest.status && newWithdrawlRequest.processedById) {
        try {
