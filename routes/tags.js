	...
	if (title && description && applicableFor) {
		try {
			...
			if (sameTag) {
				...
				let notificationDescription =
					"Another tag is having the same title and application.";
			...
			let notificationDescription =
				"Tag could not be created. Please try again after some times.";
			return res.status(500).end(notificationDescription);
		}
	} else {
	...
	if (tagId && newTag.title && newTag.description && newTag.applicableFor) {
		try {
			...
			if (conflictingTag) {
				...
				let notificationDescription =
					"Same tag exists where the title and application is as the updated tag";
				...
				try {
					...
					let notificationDescription =
						"Please check if this tag is there or not.";
					return res.status(403).end(notificationDescription);
				}
			}
		} catch (error) {
