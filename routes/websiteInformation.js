...
websiteInformationRouter.post('/create', async (req, res) => {
    const { websiteName,
            ...
            smallLogoLink,
            privacyPolicyLink,
            termsAndConditionLink } = req.body
    ...
    if (websiteName &&
        ...
        try {
            const alreadyAnInfoAvailable = await prisma.websiteInformation.findFirst()

            if (alreadyAnInfoAvailable) {
                console.log('another web info found')
                let notificationDescription = "Another website information is already there."
            ...
            let notificationDescription = "Web Info could not be created. Please try again after some times."
            return res.status(500).end(notificationDescription)
        }
    } else {
    ...
    if (
        ...
        try {
            ...
            if (sameConfiguredWebInfo) {
                ...
                let notificationDescription = "Same WebInfo exists where the configuration is as the updated WebInfo"
                ...
                try {
                    ...
                    if (updatedWebInfo) {
                        ...
                        let notificationDescription = "Please try again after sometimes. WebInfo was not updated."
                        return res.status(500).end(notificationDescription)
                    }
                } catch (error) {
                    console.log("WebInfo not found")
                    let notificationDescription = "Please check if this WebInfo is there or not."
                    return res.status(403).end(notificationDescription)
                }
            }
        } catch (error) {
