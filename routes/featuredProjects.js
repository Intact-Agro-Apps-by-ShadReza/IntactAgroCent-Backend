    ...
    if (featuredProjectsCount && projectsIdList && projectsIdList.length > 0) {
        try {
            ...
            if (createdFeaturedProjects) {
                ...
                let notificationDescription = "Featured Project was created successfully."
            ...
            let notificationDescription = "featured projects could not be created. Please try again after some times."
            return res.status(500).end(notificationDescription)
        }
    } else {
...
featuredProjectsRouter.put('/update', async (req, res) => {
    ...
    if (newFeaturedProject.featuredProjectsCount && newFeaturedProject.projectsIdList) {
        try {
            const conflictingFeaturedProjects = await prisma.featuredProjects.findFirst({
                where: {
            ...
            if (conflictingFeaturedProjects) {
                console.log("same featured projects found with the updated count and lists")
                let notificationDescription = "Same Featured Project exists where the count and the lists are as the updated Featured Project"
                ...
                try {
                    ...
                    if (featuredProject && featuredProject.id) {
                        ...
                        const updatedFeaturedProjects = await prisma.featuredProjects.update({
                            where: {
                        ...
                        if (updatedFeaturedProjects) {
                            ...
                            let notificationDescription = "Please try again after sometimes. Featured projects was not updated."
                            return res.status(500).end(notificationDescription)
                        }
                    } else {
                        const createdFeaturedProjects = await prisma.featuredProjects.create({
                    ...
                    let notificationDescription = "Please check if this featured projects is there or not."
                    return res.status(403).end(notificationDescription)
                }
            }
        } catch (error) {
