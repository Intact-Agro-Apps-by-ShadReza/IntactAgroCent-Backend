...
balanceRouter.post("/create", async (req, res) => {
	const {
		userId,
		investedMoneyInBDT,
		withdrawedMoneyInBDT,
		returnOnInvestmentMoneyInBDT,
		referredMoneyInBDT,
		extraMoneyInBDT,
		balanceInBDT,
	} = req.body;
	...
	if (
		...
		try {
			...
			if (sameBalanceOfAnUser) {
				...
				let notificationDescription =
					"Another balance is having the same user id.";
			...
			let notificationDescription =
				"balance could not be created. Please try again after some times.";
			return res.status(500).end(notificationDescription);
		}
	} else {
...
balanceRouter.put("/update", async (req, res) => {
	...
	if (
		balanceUserId &&
		newBalance.userId &&
		newBalance.investedMoneyInBDT &&
		newBalance.withdrawedMoneyInBDT &&
		newBalance.returnOnInvestmentMoneyInBDT &&
		newBalance.referredMoneyInBDT &&
		newBalance.extraMoneyInBDT &&
		newBalance.balanceInBDT
	) {
		try {
			...
			if (conflictingBalance) {
				console.log(
					"same balance found with the updated title and application"
				);
				let notificationDescription =
					"Same balance exists where the user id is as the updated balance";
				...
				try {
					...
					if (updatedbalance) {
						...
						let notificationDescription =
							"Please try again after sometimes. balance was not updated.";
						return res.status(500).end(notificationDescription);
					}
				} catch (error) {
					console.log("balance not found");
					let notificationDescription =
						"Please check if this balance is there or not.";
					return res.status(403).end(notificationDescription);
				}
			}
		} catch (error) {
