...
transactionRouter.post('/create', async (req, res) => {
    ...
    if (transactionFromId && transactionToId && transactionAmount && transactionStatus && trxId) {
        try {
            ...
            let notificationDescription = "transaction could not be created. Please try again after some times."
            return res.status(500).end(notificationDescription)
        }
    } else {
        ...
        try {
            ...
            if (oldTransaction) {
                ...
                if (updatedtransaction) {
                    ...
                    let notificationDescription = "Please try again after sometimes. transaction was not updated."
                    return res.status(500).end(notificationDescription)
                }
            } else {
                console.log("transaction id not valid")
                let notificationDescription = "Please provide valid transaction id cause it is not valid."
