...
notificationShaderRouter.post('/create', async (req, res) => {
    ...
    if (initiatedById && initiatedForType && ((initiatedForType !== "all-user" && initiatedForType !== "all-admin" && initiatedForType !== "all") && initiatedForId != "") && message) {
        try {
            ...
            if (sameNotification) {
                ...
                let notificationDescription = "Another notification is having the same configuration."
                return res.status(406).end(notificationDescription)
            } else {
                const createdNotificationShader = await prisma.notificationShader.create({
                    data: {
                ...
                if (createdNotificationShader) {
                    ...
                    let notificationDescription = "Notification shader was created successfully."
                    res.send(createdNotificationShader)
                } else {
                    console.log('notification shader not created')
                    let notificationDescription = "Notification shader could not be created."
                    return res.status(406).end(notificationDescription)
                }
            }
            ...
            let notificationDescription = "Notification shader could not be created. Please try again after some times."
            return res.status(500).end(notificationDescription)
        }
    } else {
...
notificationShaderRouter.put('/update', async (req, res) => {
    ...
    if (notificationShaderId && newNotificationShader.initiatedById && newNotificationShader.initiatedForType && newNotificationShader.initiatedForId && newNotificationShader.message) {
        try {
            const conflictingNotificationShader = await prisma.notificationShader.findFirst({
                where: {
            ...
            if (conflictingNotificationShader) {
                console.log("same notification shader found with the updated configuration")
                let notificationDescription = "Same notification shader exists where the configuration is as the updated notification shader"
                return res.status(403).end(notificationDescription)
            } else {
                try {
                    const updatedNotificationShader = await prisma.notificationShader.update({
                        where: {
                    ...
                    if (updatedNotificationShader) {
                        ...
                        let notificationDescription = "Please try again after sometimes. Notification Shader was not updated."
                        return res.status(500).end(notificationDescription)
                    }
                } catch (error) {
                    console.log("notification shader not found")
                    let notificationDescription = "Please check if this notification shader is there or not."
                    return res.status(403).end(notificationDescription)
                }
            }
        } catch (error) {
