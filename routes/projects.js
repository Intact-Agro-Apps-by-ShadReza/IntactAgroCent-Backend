		...
		try {
			...
			if (projectsCount < 1) {
				let notificationDescription =
					"Currently there are no project enlisted.";
			...
			await prisma.project
				...
				.then((response) => {
					if (response) {
						...
						let notificationDescription =
							"Please correct the url. URL not valid.";
						return res.status(404).end(notificationDescription);
					}
				})
				.catch((error) => {
	...
	if (
		...
		try {
			const sameConfiguredProject = await prisma.project.findFirst({
				where: {
					OR: [{ title: title }, { featuredPictureLink: featuredPictureLink }],
				},
			});
			...
			if (sameConfiguredProject) {
				...
				let notificationDescription =
					"Another project is having the same Title or Featured Image Link. Please have a close look.";
			...
			let notificationDescription =
				"Project could not be created. Please try again after some times.";
			return res.status(500).end(notificationDescription);
		}
	} else {
...
projectRouter.put("/update", async (req, res) => {
	...
	if (
		...
		if (
			newProject.topReturnOnInterestRate < newProject.baseReturnOnInterestRate
		) {
		...
		try {
			...
			if (
				conflictingProject &&
				conflictingProject.id !== projectId &&
				(conflictingProject.title === newProject.title ||
					conflictingProject.featuredPictureLink ===
						newProject.featuredPictureLink)
			) {
				console.log(
					"same project found with the updated title and application"
				);
				let notificationDescription =
					"Same project exists where the title, feature image, status and violation are as the updated project";
				...
				try {
					...
					if (updatedProject) {
						...
						let notificationDescription =
							"Please try again after sometimes. Project was not updated.";
						return res.status(500).end(notificationDescription);
					}
				} catch (error) {
					console.log("project not found");
					let notificationDescription =
						"Please check if this project is there or not.";
					return res.status(403).end(notificationDescription);
				}
			}
		} catch (error) {
