	...
	try {
		...
		if (foundTheEmail) {
			if (foundTheEmail.emailVerified) {
				...
				const verificationCodeForMail =
					await prisma.mailVerificationCode.findFirst({
						where: {
				...
				if (verificationCodeForMail) {
					if (currentTime <= verificationCodeForMail.expiredAt) {
						res.statusMessage =
							"A verification code is already sitting in your mail. Please verify first.";
						return res
							.status(201)
							.end(
								"A verification code is already sitting in your mail. Please verify first."
				...
				const veriificationMail = prisma.mailVerificationCode.create({
					data: {
						...
						expiredAt: new Date(
							new Date().getTime() + codeTTLInMinutes * 60000
						),
				...
				if ((await veriificationMail) && (await veriificationMail).emailId) {
					...
					transporter
						...
						.then(() => {
							res.statusMessage =
								"Password Reset code sent to your mail. Reset password using that code.";
							return res
								.status(201)
								.end(
									"Password Reset code sent to your mail. Reset password using that code."
						...
						.catch((error) => {
							...
							res.statusMessage =
								"Password Reset could not be done now. Please try again after some time.";
							return res
								.status(500)
								.end(
									"Password Reset could not be done now. Please try again after some time."
			...
			return res
				.status(403)
				.end("You are not registered. Please register first");
		}
	} catch (error) {
		...
		if (foundTheSameEmail) {
			if (foundTheSameEmail.email === email) {
				if (foundTheSameEmail.emailVerified) {
					if (
						await Encrypt.comparePassword(password, foundTheSameEmail.password)
					) {
						await prisma.mailVerificationCode.deleteMany({
							where: {
					...
					res.statusMessage =
						"Email not verified yet. An OTP has been sent to your email.";
					return res
						.status(401)
						.end("Email not verified yet. An OTP has been sent to your email.");
				}
			} else {
...
authRouter.post("/resendVerificationCode", async (req, res) => {
	...
	try {
		...
		if (foundTheEmail) {
				...
				if (currentTime <= verificationCodeForMail.expiredAt) {
					...
					if ((await veriificationMail) && (await veriificationMail).emailId) {
						...
						transporter
							...
							.then(() => {
								res.statusMessage =
									"Registration Successful. Please verify your mail. We've already sent you a verification mail to your registered Email.";
								return res
									.status(201)
									.end(
										"Registration Successful. Please verify your mail. We've already sent you a verification mail to your registered Email."
							...
							.catch((error) => {
								console.log(error.message);
								res.statusMessage =
									"Registration Successful. But mail not sent.";
								return res
									.status(500)
									.end("Registration Successful. But mail not sent.");
							});
					} else {
			...
			console.log("Email not registered");
			res.statusMessage =
				"Your Email is not registered. Please register at first.";
			return res
				.status(409)
				.end("Your Email is not registered. Please register at first.");
		}
	} catch (error) {
...
authRouter.post("/verifyMail", async (req, res) => {
	...
	try {
		...
		if (foundTheEmail) {
			...
			if (
				verificationCodeOfMail &&
				verificationCodeOfMail.expiredAt >= new Date()
			) {
...
authRouter.post("/verifyPasswordReset", async (req, res) => {
	...
	if (!(email && verificationCode && password)) {
		res.statusMessage =
			"Provide valid Email with Verification Code and New Password";
		return res
			.status(400)
			.end("Provide valid Email with Verification Code and New Password");
	}
		...
		if (foundTheEmail) {
			...
			if (
				...
				if (verificationCodeOfMail.verificationCode === verificationCode) {
					...
					if (
						verifyTheMail &&
						verifyTheMail.emailVerified &&
						verifyTheMail.password
					) {
						await prisma.mailVerificationCode.deleteMany({
							where: {
								emailId: foundTheEmail.id,
						...
						return res
							.status(200)
							.end("You have successfully reseted your password.");
					} else {
...
authRouter.post("/register", async (req, res) => {
	...
	const noVerificationNeeded =
		req.body.noVerificationNeeded === "no-verification_Needed" ? true : false;
	...
	try {
		...
		if (foundTheSameEmail) {
			...
			const emailVerificationMessage = foundTheSameEmail.emailVerified
				? "Please Signin."
				: "Please do the verification.";
			res.statusMessage =
				"This Email has already joined. " + emailVerificationMessage;
			return res
				.status(409)
				.end("This Email has already joined. " + emailVerificationMessage);
		} else {
			...
			const registeredMail = await prisma.registeredMail.create({
				data: {
					email: email,
					password: encryptedPassowrd,
					emailVerified: noVerificationNeeded,
					creationTime: new Date(),
				},
			});
			...
			if (
				...
				registeredMail.email === email
			) {
				if (noVerificationNeeded) {
					return res
						.status(201)
						.end("Mail Created With No Verification Priviledge...");
				} else {
					let config = {
						service: "gmail",
						auth: {
					...
					if (verificationCodeForMail) {
						...
						if (currentTime >= verificationCodeForMail.expiredAt) {
							...
							if (
								(await veriificationMail) &&
								(await veriificationMail).emailId
							) {
								let response = {
									body: {
										name: "Intact Agro",
				...
				return res
					.status(409)
					.end("Registration Unsuccessful. Please try again later ");
			}
		}
	} catch (error) {
		console.log("Error", error.message);
