...
transactionMessagesRouter.post('/create', async (req, res) => {
    const { sendingTime, title, description, transactionFromId, transactionToId} = req.body    
    ...
    if (title && description && transactionFromId && transactionToId) {
        try {
            ...
            if (sameTransactionMessage) {
                ...
                let notificationDescription = "Another transaction message is having the same properties."
                return res.status(406).end(notificationDescription)
            } else {
                const createdTransactionMessage = await prisma.transactionMessage.create({
                    data: {
                        title: title,
                        description: description,
                        transactionFromId: transactionFromId,
                ...
                if (createdTransactionMessage) {
                    ...
                    let notificationDescription = "Transaction mesage was created successfully."
                    res.send(createdTransactionMessage)
                } else {
                    console.log('transaction mesage not created')
                    let notificationDescription = "Rransaction mesage could not be created."
                    return res.status(406).end(notificationDescription)
                }
            }
            ...
            let notificationDescription = "Transaction mesage could not be created. Please try again after some times."
            return res.status(500).end(notificationDescription)
        }
    } else {
...
transactionMessagesRouter.put('/update', async (req, res) => {
    ...
    if (transactionMessageId && newTransactionMessage.title && newTransactionMessage.description && newTransactionMessage.transactionFromId && newTransactionMessage.transactionToId) {
        try {
            const conflictingTransactionMessage = await prisma.transactionMessage.findFirst({
                where: {
            ...
            if (conflictingTransactionMessage) {
                console.log("same transaction message found with the updated information")
                let notificationDescription = "Same transaction message exists where the information is as the updated transaction message"
                return res.status(403).end(notificationDescription)
            } else {
                try {
                    const updatedTransactionMessage = await prisma.transactionMessage.update({
                        where: {
                    ...
                    if (updatedTransactionMessage) {
                        ...
                        let notificationDescription = "Please try again after sometimes. Transactoin Message was not updated."
                        return res.status(500).end(notificationDescription)
                    }
                } catch (error) {
                    console.log("transaction message not found")
                    let notificationDescription = "Please check if this transaction message is there or not."
                    return res.status(403).end(notificationDescription)
                }
            }
        } catch (error) {
